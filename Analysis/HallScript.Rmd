...
```{r echo = FALSE, eval = F}
invisible(source_python("../Paper/postseasonNet.py"))
invisible(source_python("../Paper/seedNet.py"))
```

```{r}
post_model = read.delim("./post_model.txt")
```
...
knn_pb_2020 <- knn(train[,1:19], scale(new_test[,2:20]), cl = factor(train$POSTSEASON), k = 36, prob = TRUE)
...
knn_seed_2020 <- knn(train[,c(2:18,20:21)], knn_field_2020[,2:20], cl = factor(train$SEED), k = 6)
